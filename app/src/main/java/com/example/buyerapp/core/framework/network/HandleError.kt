package com.example.buyerapp.core.framework.network

import com.example.buyerapp.data.network.apiError
...
fun Throwable.handleThrowable(): Failure {
    ...
    return if (this is UnknownHostException) {
    ...
    } else (if (this is HttpException && this.code() == HttpStatusCode.InternalServerError.code) {
        this.apiError() ?: Failure.UnknownError
        ...
        this.apiError() ?: Failure.UnAuthorizedException
    } else if (this is HttpException && this.code() == HttpStatusCode.Forbidden.code){
        this.apiError() ?: Failure.UnknownError
